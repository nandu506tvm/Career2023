...
// Add the element to the immediate top of (1,1) and to the immedidate left of (1,1)
// Subtract the element at immediate top diagonal position of (1,1)
// Subtraction is required to avoid repetition of that diagonal element in further calcuations.
void prefixSumCalculate(vector<vector<int>> arr, vector<vector<int>> &prefix_sum, unsigned int row_size, unsigned int column_size)
{
    for (unsigned int row=1; row <= row_size; row++)
    {
        for (unsigned int column=1; column <= column_size; column++)
        {
            ...
            prefix_sum[row][column] = (arr_element + prefix_sum[row-1][column] + prefix_sum[row][column-1] ) - prefix_sum[row-1][column-1];
        }
    }
}
...
int main() 
{
    ...
    vector <vector<int>> prefix_sum(row_size+1, vector<int> (column_size+1,0));
