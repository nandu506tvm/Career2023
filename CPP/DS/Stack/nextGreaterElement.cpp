...
vector<int> nextGreaterElement(vector<int> v) {
    ...
    for (int index = length - 2; index >= 0; index--) {
        ...
        // Stack empty means no next greater element, the result is already set
        // to -1. if not empty, then current stack top is the next greater
        // element for the present index
        if (!stck.empty()) {
