 ...
 *
 * A stack is called a monotonic stack if all the elements starting from the bottom of the 
 * stack is either in increasing or in decreasing order.
 *
 * Monotonic increasing stack - pop stack top element till top is less than element to be added
 * Monotonic decreasing stack - pop stack top element till top is greater than element to add
 * Time complexity: O(n)
 *
 * Alg: Stock span = (Present element index) - (Prev greater element index)
 * Logic: From prev greater element till present element will be the span for present element.
 * For finding prev greater element use monotonic decreasing stack
 * ********************************************************************************************/
...
vector<int> stockSpanner2(vector<int> stock_prices)
{
    ...
    for (int array_index=1; array_index<length; array_index++)
    {
        ...
        while( (!stck.empty()) and (stock_prices[stck.top()] <= current_price) )
        {
            stck.pop();
        }
