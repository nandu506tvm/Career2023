...
int superDigit(string S, int k)
{
    ...
    // Note that we are passing 1 as k as we have to multiply the final result with k only once
    int subPrblmSum = superDigit(to_string(N/10),1);

    // Final calculation
    // We are multiplying by k that is if N is 148 and k is 3, then result = (1+4+8) * 3
    // Therefore result = 13*3 = 39
